...
Core API surface is built and working: auth (register/login + `GET /me`/`PATCH /update-profile`), transaction CRUD (now with an optional `categoryId`), per-user categories (CRUD), per-category budgets (CRUD, informational only, no enforcement), four report endpoints (daily/monthly/yearly/weekly, three of which now carry a `categoryBreakdown`), and AI-assisted bulk transaction parsing. There is no active multi-phase build plan — this is a maintained, already-shipped personal project, not greenfield work. The `specs/` scaffolding below exists for the _next_ piece of nontrivial work, not a backlog of already-completed phases.
...
| [08-category-management](specs/08-category-management.md)                             | Completed 2026-09-14 — see spec's own "Verify when done" note. Companion: `client/ai context/specs/12-category-management-ui.md`.                                                                             |
| [09-wire-category-to-transaction](specs/09-wire-category-to-transaction.md)           | Completed 2026-09-14, same session as `08` — see spec's own "Verify when done" note. Companion: `client/ai context/specs/13-wire-category-to-transaction-ui.md`.                                                                                                                 |
| [10-user-profile-endpoints](specs/10-user-profile-endpoints.md)                       | Completed 2026-09-14, same session as `08`/`09` — see spec's own "Verify when done" note. Companion: `client/ai context/specs/14-settings-profile-page.md`.                                                                                                               |
...
| [12-budgets](specs/12-budgets.md)                                                     | Completed 2026-09-14, same session as `08`/`09`/`10` — see spec's own "Verify when done" note. Companion: `client/ai context/specs/16-budgets-screen.md`.                                                                                                                          |
| [13-receipt-photo-upload](specs/13-receipt-photo-upload.md)                           | Completed 2026-09-14 — see spec's own "Verify when done" note below. Resolved `known-issues.md#CFG-3` as a side effect of wiring the feature it describes (`CLOUDINARY_CLOUD_NAME` now read by `config/index.ts` and set in `.env`). Companion: `client/ai context/specs/17-receipt-photo-attachment.md` (still stale against this spec's field-name revision — not touched here, out of scope).                                                    |
| [14-trend-summary-endpoint](specs/14-trend-summary-endpoint.md)                       | Completed 2026-09-14, same session as `13` — see spec's own "Verify when done" note below. Companion: `client/ai context/specs/18-trend-charts.md`.                                                                                                             |
...
## Specs 08/09/10/12 — server-side categories + settings page, completed 2026-09-14

Implemented in build order (`08` → `09` → `10` → `12`) per the user's explicit sequencing, each with its own Prisma migration applied against the real dev Neon database and its own "Verify when done" checklist fully checked off inline in the spec file — see each spec for full detail, not restated here:

- **`08`**: new `Category` model/module (`src/app/modules/category/`), mounted at `/api/categories`. Migration `20260914100201_add_category`.
- **`09`**: `Transaction.categoryId` (nullable, ownership-checked on write) + dynamic `categoryBreakdown` added to the daily/monthly/weekly summaries (`getYearlySummary` deliberately untouched). Migration `20260914100356_add_category_to_transaction`. One implementation-time type fix hit and documented in the spec itself (`ReturnType<typeof toApiShape>` on a generic function doesn't carry field types through).
- **`10`**: `GET /api/auth/me` / `PATCH /api/auth/update-profile` added to the existing `user` module — deliberately excludes email/password change/profile-picture upload (see spec's Scope for why); doesn't touch `AUTH-3` (register/login still leak the hash) per this project's "don't fix known issues as a side effect" rule.
- **`12`**: new `Budget` model/module (`src/app/modules/budget/`), mounted at `/api/budgets` — one flat monthly limit per category, **no enforcement, no notifications** (confirmed live: creating a transaction that blows past a budget's limit succeeds normally). Uses a real hard `DELETE` (this backend's first), a deliberate deviation from `Transaction`/`Category`'s soft-delete convention — see spec's Design note for why. Migration `20260914100853_add_budget`.

All four exercised live via `yarn dev` + curl against the real dev Neon database (no separate test DB exists), using two throwaway registered users (`spec08091012test@example.com`/`...test2@example.com`) and their categories/transactions/budgets as fixtures — all deleted afterward via a temporary in-tree script (`src/scripts/__tmpSpec081012Cleanup.ts`, reused the app's own `prisma` client, deleted immediately after running), same convention as spec 07's own cleanup. `yarn build` / `npx tsc --noEmit` / `yarn lint` all clean at the end of the batch — `yarn lint` shows the same 4 pre-existing errors in untouched files (`app.ts`, `Queryuilder.ts`, `interface/index.d.ts`, `globalErrorHandler.ts`) as spec 02's baseline, none new.

**Not touched, by design**: no client-side work (each spec's companion client spec is separate, unstarted); no known-issues.md items fixed as a side effect (`AUTH-3` explicitly called out and left alone per spec 10's own scope note).

## Specs 13/14 — receipt file upload + trend summary endpoint, completed 2026-09-14

Implemented together in the same session (`13` then `14`, no dependency between them beyond both touching `transaction.service.ts`/`.controller.ts`/`.route.ts`):

- **`13`**: `server/src/app/util/cloudinary.ts` (new, `deleteCloudinaryImage`/`uploadDocumentBuffer`), `server/src/app/middleware/uploadReceiptFile.ts` (new, `multer.memoryStorage()` + image-or-PDF `fileFilter` + 10MB limit), `server/src/app/interface/image.interface.ts` (new, `TCloudinaryFile`), 4 new nullable columns on `Transaction` (migration `20260914101736_add_transaction_receipt_file`), 2 new service/controller functions + `PUT`/`DELETE /api/transactions/receipt-file/:transactionId` routes with the `MulterError`-normalizing wrapper. Resolved `known-issues.md#CFG-3`: `config/index.ts` now reads `CLOUDINARY_CLOUD_NAME`/`CLOUDINARY_API_KEY`/`CLOUDINARY_API_SECRET`; `.env` updated with the real cloud name + a fresh key/secret pair the user provided mid-session (replacing the previously-dead placeholder values).
- **`14`**: new `GET /api/transactions/trend-transaction?months=N` (default 6, clamped 1-24), rolling non-calendar-year window via `getTrendSummary` in `transaction.service.ts`, reusing spec 09's `buildCategoryBreakdown` verbatim for the latest month's expense-only breakdown. No schema/migration changes.
- **Build-blocking, unrelated-to-either-spec issue found and fixed**: `@types/multer@1.4.13`'s own `"@types/express": "*"` dependency resolved (via yarn.lock, pre-existing before this session) to `@types/express@5.0.6` nested under `node_modules/@types/multer/node_modules/`, which is structurally incompatible with this project's `@types/express@^4.17.21` — made `.single("file")(req, res, cb)` fail to typecheck against this codebase's own `Request`/`Response` types. Fixed with a `"resolutions": { "@types/express": "^4.17.21" }` entry in `package.json` (yarn-only field) + `yarn install`, which removed the duplicate nested copy entirely. Not a package version bump, no new dependency — purely a dedup pin. Necessary to unblock `13`'s own routes; not a pre-existing `known-issues.md` item.

Both exercised live via `yarn dev` + curl against the real dev Neon database and the real Cloudinary account (`drmkqpdex`) using two throwaway registered users (`spec1314test@example.com`/`...test2@example.com`) and one throwaway transaction each — all deleted afterward via a temporary in-tree script (`src/scripts/__tmpSpec1314Cleanup.ts`, reused the app's own `prisma` client, deleted immediately after running), same convention as specs 07/08-12's cleanup. Receipt-file upload/replace/delete verified include confirming the *old* Cloudinary asset was actually destroyed at the origin (via a direct Cloudinary Admin API `resources` lookup, not just trusting a CDN URL — the CDN still served the old image URL with a stale `200` for a bit after `destroy()`, which is expected edge-cache behavior without `invalidate: true`, not a bug). `yarn build` / `npx tsc --noEmit` / `yarn lint` all clean at the end — `yarn lint` shows the same 4 pre-existing errors in untouched files (`app.ts`, `Queryuilder.ts`, `interface/index.d.ts`, `globalErrorHandler.ts`) as spec 02's baseline, plus one new `no-console` **warning** (not error) in the new `cloudinary.ts`, matching the existing `openRouter.ts` convention for a best-effort-catch log.

**Not touched, by design**: no client-side work (`client/ai context/specs/17-receipt-photo-attachment.md` / `18-trend-charts.md` are separate, unstarted — `17` is also still stale against `13`'s field-name revision, a pre-existing note in the spec itself, not something this session's scope covered); no known-issues.md items fixed beyond `13`'s own explicitly-flagged `CFG-3` side effect.
...
3. Client companion specs `17-receipt-photo-attachment.md` (needs a field-name-matching pass against `13`'s revision first) and `18-trend-charts.md` are unstarted.
