...
## Pending: independent feature proposals for `client`/`server`

`feature-plan-proposals.md` (this root) is a **proposed, not-yet-approved** set of new-feature ideas for ExpenseTracker — categories, per-category budgets, recurring transactions, receipt photo attachments, charts/trends, CSV/PDF export, and AI spending insights — written for the user to review and decide which (if any) to pursue, in what order. **Read that file before doing any feature-planning or implementation work here** — it's the source of truth for the proposals, not this summary.

Status as of 2026-09-14: proposals only, presented for review, not yet discussed/approved by the user, no spec created yet in either `client/ai context/specs/` or `server/ai context/specs/`, no code touched. If the user asks to start or continue on any of these in a fresh session:
1. Re-read `feature-plan-proposals.md` in full first — it covers, per feature, the problem it solves, a usage scenario, an implementation sketch (server + client), effort estimate, and dependencies, plus a sequencing diagram (Categories is foundational — it unlocks/improves Budgets, Charts, and AI insights; Recurring transactions, Receipt photos, and Export are independent of everything else).
2. Confirm with the user whether the proposals (or any part of them) have since changed, been approved, or been partially implemented — this file may be stale relative to the actual conversation that happens after it, since it was written before any implementation started.
3. These proposals are explicitly scoped to ExpenseTracker only — no dependency on, or feature involving, Bike Log (a separate project by the same developer at `../22 bike log app initial /`). Cross-project features and Bike Log's own independent features were deliberately excluded from this doc per direct user instruction; don't reintroduce them unless the user asks.
4. Don't promote a proposal into a real numbered spec until the user has actually chosen to build it — per `client/ai context/specs/00-build-plan.md` / `server/ai context/specs/00-build-plan.md`'s own convention, a spec gets created only when work is about to actually start, not speculatively ahead of time.
