...
| FETCH-7 | Medium | The response error interceptor derives its Toast message with `const errorMessage = error?.response?.data?.message;` — for any error with no HTTP response at all (offline, timeout, DNS failure, server unreachable), `error.response` is `undefined`, so `errorMessage` is `undefined`. | `utils/axiosInstance.ts:70-76` | `Toast.show({ text1: undefined, ... })` renders a visible error-colored toast container with no message text at all. Reported directly by the user while testing offline behavior (pull-to-refresh on Home with no connection): the toast appears but is silently blank, giving no indication of what went wrong. Note this is distinct from `FETCH-1` — it fires specifically for non-HTTP (no-`response`) failures, not for ordinary 4xx/5xx errors, which do have `response.data.message`. |
...
| UX-3 | Medium | `react-native-web`'s `Alert` export is a hard stub (`node_modules/react-native-web/dist/exports/Alert/index.js`: `class Alert { static alert() {} }`) — every `Alert.alert(...)` call silently no-ops on the web target only (native iOS/Android unaffected). Fixed for the Home logout button (branches on `Platform.OS === "web"` to use `window.confirm` there instead — see `specs/08-discoverable-logout.md`'s follow-up fix) but every other call site is still exposed: tapping the control does nothing on web, so the destructive action it was supposed to confirm never happens either. | `components/main/shared/TransactionCard.tsx` (delete confirm), `components/main/shared/PendingTransactionEditModal.tsx` (pending-delete confirm), `components/main/smartAdd/SmartAdd.tsx` (remove-draft confirm) |
