...
Supersedes `15-transaction-categories-superseded.md`'s client-side design — see that file's superseded banner. The key difference from that discarded design: categories now come from a live `GET /categories` fetch (via the generic `useFetchData` from `client/hooks/useApi.ts`, spec 12's user-owned list — no dedicated `useCategories.ts` wrapper hook, per direct user instruction on spec 12), not a hardcoded `constants/Category.constant.ts` with 7 fixed values.

## Goal

Let the user tag a transaction with one of _their own_ categories when adding/editing it, see that category at a glance in transaction lists, and see a per-category spend breakdown (with tap-to-filter) on the Monthly/Weekly overview.

## Scope

**In scope:**

- A `CategoryPicker` component (chip grid, sourced from a direct `useFetchData` call against `/categories`) added to `AddTransactionPage.tsx` and `UpdateTransactionModal.tsx`. Selection is optional — a transaction with no category is valid (`categoryId: null` / "Uncategorized"), unlike the discarded spec `15`'s design which always defaulted to a fixed `"other"` value.
...
- **Category on offline-queued transactions — per direct user instruction (2026-09-14), the offline path never sends a category.** `categoryId` is deliberately **not** added to `TPendingTransactionPayload`/`transactionQueue.ts`, and `PendingTransactionEditModal.tsx` gets no `CategoryPicker`. `AddTransactionPage.tsx`'s offline-enqueue payload omits `categoryId` entirely, so any transaction added while offline always syncs as Uncategorized; the user can add a category afterward via the normal edit flow once it's synced.
...
`useFetchData` and `TCategory` are imported directly (`@/hooks/useApi`, `@/types/Category.types`) — no dedicated category hook, per direct user instruction on spec 12, carried through here.
...
### 3. Wire into the online add/edit forms

Same integration points as the discarded spec `15` for the two forms that always go straight to the server, now storing `categoryId: string | null` instead of a `TCategory` enum value:

- **`AddTransactionPage.tsx`**: `const [categoryId, setCategoryId] = useState<string | null>(null)`. Render `<CategoryPicker value={categoryId} onChange={setCategoryId} />` after `<TypeToggle .../>`. Include `categoryId` in the manual-add (online `POST`) payload only — **per direct user instruction (2026-09-14), the offline-enqueue payload never includes `categoryId`**, so the two payload-building branches diverge on this one field (see §4). Reset `categoryId` to `null` alongside the other field resets on success. **Category stays optional at the form level too**: the existing `PrimaryButton`'s `disabled={!title || !amount}` condition (`AddTransactionPage.tsx`) is left exactly as-is — `categoryId` is deliberately _not_ added to it, so a transaction saves fine with no category picked, the same way it already does with no description.
- **`UpdateTransactionModal.tsx`**: `categoryId` state initialized from `initialValue?.categoryId ?? null`, synced in the existing `useEffect`, included in the PATCH payload — this modal only ever edits transactions that already exist server-side, so it's unaffected by the offline-queue decision below.
- **`PendingTransactionEditModal.tsx`**: **out of scope** — see §Out of scope. No `CategoryPicker` here; offline-queued items carry no category to edit.

### 4. Offline queue — `client/utils/transactionQueue.ts` (unchanged)

**No changes to this file.** Per direct user instruction (2026-09-14), category is not part of the offline path at all: `TPendingTransactionPayload` gets no `categoryId` field, and `AddTransactionPage.tsx`'s offline-enqueue branch never reads the picker's value. A transaction added while offline always syncs as Uncategorized; the user adds a category afterward through the normal (now-online, server-backed) edit flow if they want one. This sidesteps the whole class of "category picked offline, then deleted server-side before sync" conflict the earlier draft of this spec had flagged as a caveat — it's moot now, since offline items never carry a `categoryId` to begin with.
...
- `client/components/main/AddTransaction/AddTransactionPage.tsx` (edit — picker + online-payload field only, offline-enqueue payload untouched)
...
Not touched (explicitly out of scope): `client/components/main/shared/PendingTransactionEditModal.tsx`, `client/utils/transactionQueue.ts` — no category on the offline path, per direct user instruction (2026-09-14).
...
- [ ] A transaction added while offline shows no category picker in `PendingTransactionEditModal.tsx`, and once synced, lands server-side as Uncategorized (`categoryId: null`) — confirms the offline path never sends a category, per direct user instruction (2026-09-14).
...
- [ ] Deleting a category (via spec 12) that some past transactions still reference: those transactions keep showing the category's name/icon (server spec 09's soft-delete design), and the picker on _new_ transactions no longer offers it.
