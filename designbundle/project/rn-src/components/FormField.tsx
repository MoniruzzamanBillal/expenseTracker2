...
export function FormField({ label, error, passwordToggle, secureTextEntry, inputStyle, ...inputProps }: Props) {
  const C = useTheme();
  const [hidden, setHidden] = useState(secureTextEntry ?? false);

  return (
    <View style={label ? s.wrap : undefined}>
      {label ? (
        <Text style={[text.label, { color: C.textSecondary, marginBottom: spacing.xs }]}>
          {label.toUpperCase()}
        </Text>
      ) : null}
      <View style={[
        s.inputWrap,
        { backgroundColor: C.inputBg, borderColor: error ? C.expense : C.border },
      ]}>
        <TextInput
          {...inputProps}
          secureTextEntry={passwordToggle ? hidden : secureTextEntry}
          style={[s.input, text.body, { color: C.text }, inputStyle]}
          placeholderTextColor={C.placeholder}
        />
        {passwordToggle ? (
          <TouchableOpacity onPress={() => setHidden(h => !h)} style={s.eyeBtn}>
            <Text style={{ fontSize: 16, color: C.textMuted }}>{hidden ? '○' : '●'}</Text>
          </TouchableOpacity>
        ) : null}
      </View>
      {error ? (
        <Text style={[text.caption, { color: C.expense, marginTop: spacing.xs }]}>{error}</Text>
      ) : null}
    </View>
  );
}
